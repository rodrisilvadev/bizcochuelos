...
import { Scale, ChevronDown, Info, AlertTriangle } from 'lucide-react';
...
// historial en lugar de acumularse.
export const BalanceLevadura: React.FC<BalanceLevaduraProps> = ({ ledger, currentUser }) => {
    ...
    <div className="animate-fade-up delay-150">
      <div className="flex items-center justify-between mb-3 px-1">
        <div className="flex items-center gap-2">
          <Scale className="w-4.5 h-4.5 text-apple-green" strokeWidth={2.5} />
          <div>
            <span className="text-sm font-extrabold text-carbon-dark dark:text-white">Balance de Levadura</span>
            <p className="text-[11px] text-gray-400 font-semibold leading-none mt-0.5">la verdad dura</p>
          </div>
        </div>
        <button
          ...
          className={`w-8 h-8 rounded-xl border flex items-center justify-center transition-all cursor-pointer flex-shrink-0 ${
            showHelp
      ...
      {showHelp && (
        <div className="mb-3 rounded-2xl bg-carbon-light dark:bg-white/5 border border-gray-100 dark:border-white/10 px-4 py-3.5 space-y-2.5 animate-scale-up">
      ...
      {!hasData ? (
        ...
        <div className="bg-white dark:bg-carbon-gray rounded-3xl border border-gray-100 dark:border-white/10 shadow-card overflow-hidden">
          {imbalance !== 0 && (
            <div className="flex items-start gap-2 px-5 py-3 bg-amber-50 dark:bg-amber-500/10 border-b border-amber-100 dark:border-amber-500/20">
          ...
          <button
            onClick={() => setShowHelp(h => !h)}
            className="w-full px-5 py-3 flex items-center justify-center gap-1.5 bg-carbon-light/40 dark:bg-white/5 border-t border-gray-100 dark:border-white/10 text-[10px] text-gray-400 font-semibold hover:text-carbon-dark dark:hover:text-white transition-colors cursor-pointer"
          >
            <span>El más negativo es el que sigue en la cola</span>
